jQuery(document).ready(function() {
    ...
    function clearSchoolGroupChoices(elt) {
        ...
        jQuery(elt).find('.control-group.schoolgroup table')
            .find('tr.content-row').remove();
        jQuery(elt).find('.control-group.schoolgroup select')
    ...
    function clearSchoolChoices(elt) {
        ...
        jQuery(elt).find('.control-group.school').removeClass('error');
        jQuery(elt).find('select[name="school"]')
            .find('option').not('.all-or-none-option').remove();
    }
                ...
                if (json.groups.length < 1) {
                    ...
                    if (eltGroup.tagName === 'SELECT') {
                        for (i = 0; i < json.groups.length; i++) {
                            group = json.groups[i];
                            var option = '<option value="'  + group.id + '">' +
                                group.name + '</option>';
                        ...
                        for (i = 0; i < json.groups.length; i++) {
                            ...
                            if (group.member) {
                                ...
                                choice += '<td><form action="/join_group/" ' +
                                    'method="post">' +
                                    '<button class="btn btn-primary ' +
                                    'btn-small">Join</button>' +
    ...
    jQuery('button.edit-group-button').click(function() {
        ...
        jQuery(modal)
            .find('input[name="start_date"]').datepicker('setDate', startdate);
        jQuery(modal)
            .find('input[name="end_date"]').datepicker('setDate', enddate);
        jQuery(modal).modal();
    });
    ...
    jQuery('form.create-group, form.edit-group').submit(function(evt) {
        ...
        if (startdate === 'Invalid Date') {
            jQuery(this)
                .find('div.control-group.start_date')
                .addClass('error');
            submit = false;
        }
    ...
    if (profile_attributes.role === 'faculty' ||
        ...
        var eltCountrySelect =  jQuery(containers)
            .find('select[name="country"]');
        var eltSchoolSelect =  jQuery(containers)
            .find('select[name="school"]');

        jQuery(eltCountrySelect).val(profile_attributes.country);
        jQuery(eltCountrySelect).attr('disabled', 'disabled');

        populateSchoolChoices(
            jQuery(containers),
